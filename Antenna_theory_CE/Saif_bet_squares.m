...
% Here Einc is the desired field on the surface of the dipole, so minus
% sign in the exponent means invertsion of the incoming wave 

k = Coordinate(1,0,0); % define k of the incoming wave

Einc = Vector_field(Function3(@(c1,c2,c3)0),...
                    Function3(@(c1,c2,c3)0),...
                    Function3(@(c1,c2,c3)-exp(-1i*(k*Coordinate(c1,c2,c3)))),...
                    'cart');
...
%% Define normals and tangentials to dipole surface

% Side_normals = Vector_field(Function3(@(c1,c2,c3)cos(c2)),...
%                             Function3(@(c1,c2,c3)sin(c2)),...
%                             Function3(@(c1,c2,c3)0), 'cyl'); % vector field of normals to dipole side surface
%                         
% Side_span1 = Vector_field(Function3(@(c1,c2,c3)0),...
%                           Function3(@(c1,c2,c3)0),...
%                           Function3(@(c1,c2,c3)1), 'cyl'); % vector field of tangentials to dipole side surface
% 
%     
% Side_span2 = Vector_field(Function3(@(c1,c2,c3)-sin(c2)),...
%                           Function3(@(c1,c2,c3)cos(c2)),...
%                           Function3(@(c1,c2,c3)1), 'cyl'); % vector field of tangentials to dipole side surface
...
parfor n = 1:N_currents % create array of E-field functions generated by base currents located on the side of the dipole
    ...
    f_string = sprintf('@(c1,c2,c3)(c3 >= %.15f)*(c3 < %.15f)', currents_limits(n), currents_limits(n+1));
    ...
    Current_A = Green_integral(Green, current_J, 'no', 0, 0, currents_limits(n), 0, 0, currents_limits(n+1), 1, 1, N_height_in_support, 'cart'); % Compute vector potential produced by z current
...
parfor n = 1:N_weights
    ...
    f_string = sprintf('@(c1,c2,c3)(c3 >= %.15f)*(c3 < %.15f)', weights_limits(n), weights_limits(n+1));
...
for m = 1:N_weights % calculate members of z matrx and results vector
    ...
    N_height_in_support = ceil((weights_limits(m+1)-weights_limits(m)) / (2*half_l/N_height));
    
    Result_col(m) = Integral(Einc * Weights{m}, R, 0, weights_limits(m), R, 2*pi*(N_phi-1)/N_phi, weights_limits(m+1), 1, N_phi, N_height_in_support, 'cyl');

    parfor n = 1:N_currents
        
        Z_Matrix(m,n) = Integral(Es{n} * Weights{m}, R, 0, weights_limits(m), R, 2*pi*(N_phi-1)/N_phi, weights_limits(m+1), 1, N_phi, N_height_in_support, 'cyl');
